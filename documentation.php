<!DOCTYPE html>
<html lang="fr">
<head>
	<meta charset="utf-8">
	<link rel="stylesheet" type="text/css" href="./style/documentation_stylesheet.css">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Le Labo Collectif - Documentation</title>
</head>
<body>
    <div id="bloc_page">

        <?php include_once('header.php'); ?>

            <section id="ateliers_vivants">
                <div id="av1">
                    <div class="bandeau_transparent">
                        <a href="./images/pdf/plaquette_ateliersvivants.pdf">Télécharger la plaquette</a>
                    </div>
                </div>
            </section>

            <section id="plaquettes">   
                <div id="mediation">
                    <div class="bandeau_transparent">
                        <a href="./images/pdf/plaquette_mediation.pdf">Télécharger la plaquette</a>
                    </div>
                </div>
                <div id="sensibilisation">
                    <div class="bandeau_transparent">
                        <a href="./images/pdf/plaquette_sensibilisation.pdf">Télécharger la plaquette</a>
                    </div>
                </div> 
            </section>

            <div id="support_link">
                <a href="./images/pdf/support_mediation.pdf">Télécharger notre support de médiation</a>
            </div>

        <?php include_once('footer.php'); ?>


    </div>

</body>
</html>